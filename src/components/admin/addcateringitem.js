    ...
    try {
      ...
      if (imageFile) {
        ...
        const storageRef = ref(
          storage,
          `catering/${timestamp}-${imageFile.name}`
        );
      ...
      alert("Catering product added successfully!");
      ...
      >
        ...
        <div className=" col-span-2">
          ...
          <textarea
            ...
            onChange={handleChange}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
            placeholder="e.g., Creamy Alfredo sauce with tender grilled chicken..."
            rows="3"
          />
          {errors.description && (
        ...
        <button
          type="submit"
          className="w-full col-span-2 px-6 py-3 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
          disabled={loading}
        >
          {loading ? "Uploading & Adding..." : "Add Catering Product"}
        </button>
      </form>
    </div>
  );
